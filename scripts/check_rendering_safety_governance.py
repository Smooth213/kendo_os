...
def run_rendering_safety_governance():
    ...
    rules = [
        ("1. 運営・管理系画面 (12画面) UIゼロレンダリングエラー保証", "1. 運営・管理系画面"),
        ("2. 運営試合・スコア系画面 (7画面) UIゼロレンダリングエラー保証", "2. 運営試合・スコア系画面"),
        ...
        ("6. ドック全ボトムシート展開 (18シート) UIゼロレンダリングエラー保証", "6. ドック全ボトムシート展開"),
    ...
    print("-" * 60)
    if passed_all:
        print(" 🟢 監査結果: 合格 (全35画面・全18ボトムシートのゼロレンダリングエラーが完全に保証されています！)")
        print("=" * 60)
        sys.exit(0)
    else:
