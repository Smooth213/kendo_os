...
void main() {
  ...
  final factoryContent = factoryFile.existsSync()
      ? factoryFile.readAsStringSync()
      : '';
  final resolverContent = resolverFile.existsSync()
      ? resolverFile.readAsStringSync()
      : '';
  ...
  for (final file in ruleFiles) {
    ...
    if (fileName == 'rule_factory' ||
        fileName == 'match_rule_interface' ||
        fileName == 'rule_preset' ||
    ...
    if (!allUsageContext.contains(assumedClassName) &&
        !allUsageContext.contains(fileName)) {
      print(
        '⚠️ [Dead Rule Warning] ルール "$assumedClassName" (${file.path}) は RuleFactory や Engine に登録されていない可能性があります。',
      );
  ...
  if (hasDeadRule) {
    print(
      '💡 ガバナンス維持のため、未使用のルールは削除するかアーカイブしてください (see: governance_lifecycle_policy.md)。',
    );
  } else {
