...
class MatchDomainService {
  ...
  /// 勝ち抜き戦の次の試合を生成する
  MatchModel? generateNextKachinukiMatch(
    ...
    if (finishedMatch.redScore == finishedMatch.whiteScore) {
      if (nextRedRem.isEmpty && nextWhiteRem.isEmpty) {
        if (rule.kachinukiUnlimitedType == '大将引き分け延長' &&
            finishedMatch.matchType != '大将延長戦') {
          isMatchOver = false;
          isEncho = true;
        } else {
          isMatchOver = true;
    ...
    if (!isMatchOver) {
      return MatchModel(
        ...
        category: finishedMatch.category,
        groupName: finishedMatch.groupName,
        matchType: isEncho ? '大将延長戦' : '勝ち抜き戦',
        redName: nextRedName,
        whiteName: nextWhiteName,
        status: 'waiting',
        matchTimeMinutes: finishedMatch.matchTimeMinutes,
        isRunningTime: finishedMatch.isRunningTime,
        order: finishedMatch.order + 0.1,
        note: isEncho ? '延長戦（1本勝負）' : finishedMatch.note,
        isKachinuki: true,
        redRemaining: nextRedRem,
        whiteRemaining: nextWhiteRem,
      );
    }
    return null;
  }
  ...
  List<MatchModel> propagateNameToNextMatches(
    MatchModel finishedMatch,
    ...
    final winnerName = isRedWin
        ? finishedMatch.redName
        : finishedMatch.whiteName;
    final loserName = isRedWin
        ? finishedMatch.whiteName
        : finishedMatch.redName;
    ...
    for (var m in allMatches) {
      if (m.status != 'finished') {
        ...
        if (nextRed.contains(winnerTag)) {
          nextRed = nextRed.replaceFirst(winnerTag, winnerName);
          updated = true;
        }
        if (nextRed.contains(loserTag)) {
          nextRed = nextRed.replaceFirst(loserTag, loserName);
          updated = true;
        }
        if (nextWhite.contains(winnerTag)) {
          nextWhite = nextWhite.replaceFirst(winnerTag, winnerName);
          updated = true;
        }
        if (nextWhite.contains(loserTag)) {
          nextWhite = nextWhite.replaceFirst(loserTag, loserName);
          updated = true;
        }
        if (updated) {
          updatedMatches.add(
            m.copyWith(redName: nextRed, whiteName: nextWhite),
          );
        }
      }
    }
