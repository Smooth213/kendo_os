...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  _buildSectionFooter(context, 'ログアウトすると現在のセッションが終了し、次回利用時に再ログインが必要になります。'),
                  const SizedBox(height: 24),
                ],
              ),
            ),
  ...
  List<Widget> _buildHiddenAdminSections(BuildContext context) {
    ...
    return [
      ...
      // 内部特権領域 (Admin専用機能)
      // ==========================================
      if (FeatureGate.canUseAI(currentRole) || FeatureGate.canAccessMetrics(currentRole)) ...[
        _buildSectionHeader(context, '内部特権・開発機能 (Admin専用)'),
        _buildSettingsBlock(context, [
          if (FeatureGate.canUseAI(currentRole))
            _buildSwitchTile(context, 'AI自動判定・ガバナンスエンジン', false, (val) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Stage2 β版では設定固定されています')));
            }, icon: Icons.smart_toy, iconBgColor: Colors.deepPurple),
          if (FeatureGate.canAccessMetrics(currentRole))
            _buildSwitchTile(context, '内部メトリクス・ダッシュボード', false, (val) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Stage2 β版では設定固定されています')));
            }, icon: Icons.analytics, iconBgColor: Colors.blueGrey),
        ]),
        _buildSectionFooter(context, 'これらの機能はシステム管理者のみに解放されています。一般ユーザーや運用者の画面には表示されません。'),
      ],
    ];
  }
}