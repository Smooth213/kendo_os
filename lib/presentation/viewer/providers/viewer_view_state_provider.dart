...
final viewerMatchProjectionProvider = StreamProvider.family<MatchProjection?, String>((ref, matchId) async* {
  ...
  // =========================================================================
  if (kIsWeb) {
    debugPrint('🌐 [Viewer Web Bypass] Web環境のため、クラウドから対象の試合を直接監視してProjectionへ変換します: $matchId');
    ...
    final stream = firestore.collection('organizations').doc(dojoId).collection('matches').doc(matchId).snapshots();
...
final viewerTournamentProjectionProvider = Provider.family<AsyncValue<TournamentProjection?>, String>((ref, tournamentId) {
  ...
  if (targetMatches.isEmpty && kIsWeb) {
    final webMatchesAsync = ref.watch(matchListByTournamentProvider(tournamentId));
  ...
  final tournament = tournamentAsync.value ?? TournamentModel(
    id: tournamentId,
    organizationId: '',
    name: '大会情報',
    date: DateTime.now(),
        ...
        .map((snapshot) {
          return snapshot.docs.map((doc) {
            try {
              ...
              debugPrint('⚠️ [bunaiksenMatchesProvider] Error parsing match ${doc.id}: $e\n$stack');
              return null;
            }
