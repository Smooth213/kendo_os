...
import 'package:kendo_os/shared/infrastructure/repository/match_repository.dart'; // ★ 復元
import 'package:kendo_os/features/match/domain/services/kendo_rule_engine.dart'; // ★ 復元         // ★ 復元
...
final currentMatchIdProvider =
    NotifierProvider<CurrentMatchIdNotifier, String?>(() {
...
class MatchActionController {
  ...
  Future<void> processScoreEvent(
    MatchModel currentMatch,
    ScoreEvent event,
  ) async {
    await ref
        .read(matchApplicationServiceProvider)
        .addIppon(currentMatch.id, event.side, event.type);
  }
  ...
  Future<void> handleTimeUp(
    MatchModel currentMatch,
    bool isEnchoEnabled,
  ) async {
    await ref
        .read(matchApplicationServiceProvider)
        .handleTimeUp(currentMatch.id);
  }
