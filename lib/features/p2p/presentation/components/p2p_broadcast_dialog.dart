...
class _P2pBroadcastDialogState extends ConsumerState<P2pBroadcastDialog> {
  ...
  void _fallbackToCloudUrl() {
    final dojoId = ref.read(currentDojoIdProvider).isNotEmpty
        ...
        : 'default_org';
    final tournamentId =
        (widget.match.tournamentId != null &&
            widget.match.tournamentId!.isNotEmpty)
        ? widget.match.tournamentId!
        : ref.read(currentTournamentIdProvider);

    final isDantai =
        widget.match.matchType.contains('団体') ||
        (widget.match.groupName != null &&
            widget.match.groupName!.isNotEmpty &&
            !widget.match.matchType.contains('個人'));
    ...
    if (isDantai &&
        widget.match.groupName != null &&
        widget.match.groupName!.isNotEmpty) {
      path = widget.match.isKachinuki ? 'viewer-kachinuki' : 'viewer-team';
      targetId = widget.match.groupName!;
    } else {
      path = 'viewer';
    ...
    final cloudUrl =
        'https://kendo-os-beta.web.app/$path/${Uri.encodeComponent(targetId)}?$queryString';
    setState(() {
      _serverUrl = cloudUrl;
      _isLocalP2p = false;
      _isLoading = false;
