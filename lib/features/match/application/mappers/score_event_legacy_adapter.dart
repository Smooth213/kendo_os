...
class ScoreEventLegacyAdapter {
  ...
  static bool verifySignature(ScoreEvent event, String secret) {
    ...
    final payload =
        '${event.id}:$uid:${event.timestamp.toIso8601String()}:${event.side.name}:${event.type.name}';
  ...
  static ScoreEvent fromLegacy({
    ...
    final payload =
        '$eventId:$uid:${time.toIso8601String()}:${side.name}:${type.name}';
