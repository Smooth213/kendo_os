...
import 'package:kendo_os/features/match/domain/rules/rule_preset.dart'; // ★ Phase 7: プリセットのインポート
import 'package:kendo_os/features/match/domain/rules/rule_config_validator.dart'; // ★ Phase 8: インポートを追加
...
final ruleSummaryProvider = Provider<String>((ref) {
  ...
  final timeStr =
      '${c.time.matchTimeMinutes.toStringAsFixed(c.time.matchTimeMinutes % 1 == 0 ? 0 : 1)}分';
  final scoreStr = c.scoring.isIpponShobu
      ? '1本勝負'
      : '3本勝負(${c.scoring.ipponLimit}本先取)';
  final enchoStr = c.encho.isEnchoUnlimited
      ? '延長無制限'
      : (c.encho.enchoCount > 0
            ? '延長${c.encho.enchoCount}回(${c.encho.enchoTimeMinutes.toInt()}分)'
            : '延長なし');
  ...
  if (errors.isNotEmpty) {
    summary +=
        '\n\n❌ 以下の設定エラーを修正してください：\n${errors.map((e) => '・$e').join('\n')}';
  } else if (!c.encho.isEnchoUnlimited &&
      c.encho.enchoCount == 0 &&
      !c.draw.hasHantei &&
      !c.scoring.isIpponShobu) {
