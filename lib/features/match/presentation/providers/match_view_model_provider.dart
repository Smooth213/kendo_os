...
final tournamentMatchesProvider = Provider.family
...
final activeMatchesProvider = Provider.family
    .autoDispose<ActiveMatches, String>((ref, tournamentId) {
      ...
      for (var m in matches) {
        if (m.groupName != null && m.groupName!.isNotEmpty) {
          ...
          final groupMatches = matches
              .where((gm) => gm.groupName == m.groupName)
              .toList();
          if (groupMatches.any((gm) => gm.status == 'in_progress')) {
            uniqueInProgress.add(m);
          } else if (groupMatches.every((gm) => gm.status == 'waiting')) {
            uniqueWaiting.add(m);
          }
        } else {
...
final timelineMatchesByCategoryProvider = Provider.family
    .autoDispose<TimelineMatchesResult, String>((ref, tournamentId) {
      final matches = ref.watch(tournamentMatchesProvider(tournamentId));
      ...
      int getWeight(String cat) {
        ...
        if (cat.contains('大学') || cat.contains('一般') || cat.contains('シニア')) {
          return 60;
        }
        return 999;
      }
...
final bunaiksenRecordCategoryGroupsProvider = Provider.family
    .autoDispose<Map<String, Map<String, List<MatchModel>>>, String>((
      ref,
      tournamentId,
    ) {
      final matches = ref.watch(tournamentMatchesProvider(tournamentId));
      ...
      for (var m in matches) {
        ...
        final groupKey = (m.groupName != null && m.groupName!.isNotEmpty)
            ? m.groupName!
            : '__default__';
