...
class TimelineExportService {
  ...
  static String exportToTxt(List<MatchModel> matches, String tournamentName) {
    ...
    for (int i = 0; i < matches.length; i++) {
      ...
      buffer.writeln(
        ' 対戦: ${m.redName} [${m.redScore}] vs [${m.whiteScore}] ${m.whiteName}',
      );
      ...
      if (m.events.isEmpty) {
        ...
        for (var e in m.events) {
          ...
          buffer.writeln(
            '時刻: ${e.timestamp.toLocal().hour}:${e.timestamp.toLocal().minute.toString().padLeft(2, '0')} | 側: $side | 内容: ${e.type.toString().split('.').last.toUpperCase()}',
          );
        }
      }
      buffer.writeln('\n==================================================\n');
