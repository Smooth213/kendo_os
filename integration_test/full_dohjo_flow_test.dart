...
void main() {
  group('🛡️ フェーズ6 — 実機フロー・オフライン復帰 統合テスト要塞', () {
    test(
      '1. 【道場運営フロー】ログインから試合開始、スコア入力、Viewer反映にいたる一連のバリューチェーンが破綻なく貫通すること',
      () async {
        // MatchBuilderによる道場運営データのモック生成
        final managedMatch = MatchBuilder()
            .id('dohjo_flow_999')
            .inProgress()
            .build();
    ...
    test(
      '2. 【オフライン復帰・Web再接続】通信切断状態でローカル保存したデータが、オンライン復帰時にクラウド側と安全に自動同期（Merge）されること',
      () async {
        final offlineMatch = MatchBuilder()
            .id('dohjo_offline_777')
            .localOnly()
            .build();
