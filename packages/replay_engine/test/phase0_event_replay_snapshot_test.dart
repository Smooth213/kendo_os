  ...
  group('📼 Phase 0-3: Event Replay Snapshot (履歴の完全再生比較)', () {
    test('100試合のランダムイベントを処理し、最終状態のチェックサムが一致すること', () {
      ...
      final testUser = const User(
          id: 'snapshot_user', role: Role.admin, organizationId: 'test_org');
      ...
      for (int i = 0; i < 100; i++) {
        var match =
            TestMatchFactory.createIndividualMatch(id: 'replay-match-$i');
      ...
      expect(totalRedScoreChecksum, 95,
          reason: '赤の総スコア合計(Checksum)が壊れています。リファクタリングによるデグレが発生しました。');
      expect(totalWhiteScoreChecksum, 132,
          reason: '白の総スコア合計(Checksum)が壊れています。リファクタリングによるデグレが発生しました。');
      expect(totalFinishedMatches, 88,
          reason: '終了した試合数(Checksum)が壊れています。リファクタリングによるデグレが発生しました。');
    });
    ...
    test('0-2: Rule JSON Snapshot (入力イベントと期待される結果の固定化)', () {
      ...
      final addScoreUseCase = AddScoreUseCase(engine, permission, timeSource);
      final testUser = const User(
          id: 'json_user', role: Role.admin, organizationId: 'test_org');
      final rule = const MatchRule();
      ...
      final snapshot = {
        "events": match.events
            .map((e) => {'type': e.type.name, 'side': e.side.name})
            .toList(),
        "expectedResult": {
    ...
    test('Phase 8-1, 8-2, 8-3: 体育館電波断・タイマーNaN汚染・プロセスKillからの決定論的Snapshot再生復旧テスト',
      ...
      final addScoreUseCase = AddScoreUseCase(engine, permission, timeSource);
      final testUser = const User(
          id: 'chaos_v8_user', role: Role.admin, organizationId: 'test_org');

      // 1. 【手順 8-2 耐久検証】: タイマーに Infinity や NaN などの異常値（省電力モード時の端数クラッシュ要因）を強制注入
      var chaosMatch =
          TestMatchFactory.createIndividualMatch(id: 'chaos-match-v8').copyWith(
        matchTimeMinutes: double.nan, // NaNを意図的にパルス注入
      );

      // 判定ロジックが自爆クラッシュ（toInt 例外）を起こさず、安全防壁が機能して0秒（タイムアップ）として処理されることを検証
      final remainingSeconds =
          chaosMatch.calculateRemainingSeconds(DateTime.now());
      expect(remainingSeconds, 0,
          reason: '【手順8-2】タイマーNaN汚染時に自爆クラッシュを弾く防衛線が機能していません。');
      ...
      for (int i = 0; i < 100; i++) {
        var matchState =
            TestMatchFactory.createIndividualMatch(id: 'chaos-sim-$i');
        ...
        for (int j = 0; j < 8; j++) {
          ...
          final event = ScoreEventLegacyAdapter.fromLegacy(
              side: side, type: type, sequence: j, userId: testUser.id);

          matchState = addScoreUseCase.execute(
              testUser, matchState, event, const MatchRule());
        }
      ...
      for (final matchId in simulatedIsarDisk.keys) {
        ...
        final hydratedModel =
            MatchModel.fromJson(jsonDecode(rawJson) as Map<String, dynamic>);
      ...
      debugPrint(
          '🏁 [Chaos Final Checksum] Red: $redScoreAccumulator | White: $whiteScoreAccumulator');

      expect(redScoreAccumulator, 122,
          reason:
              '【手順8-1】ライトスルー書き込み、または【手順8-3】プロセスKill復旧時に赤のスコアにリプレイ・ドリフト（歴史のズレ）が発生しました。');
      expect(whiteScoreAccumulator, 127,
          reason:
              '【手順8-1】ライトスルー書き込み、または【手順8-3】プロセスKill復旧時に白のスコアにリプレイ・ドリフト（歴史のズレ）が発生しました。');
    });
  });
}
