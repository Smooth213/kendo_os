...
void main() {
  group('🛡️ Phase 8 — 大規模性能試験・パフォーマンスゲート要塞', () {
    ...
    test('1. 【Projection性能】3000試合の巨大データ配列に対するメモリ検索・フィルタリング構築時間が、Performance Gate規定の「200ms」未満で高速完了すること', () {
      final massiveMatches = List.generate(3000, (index) => MatchModel(
      ...
      final projectionResult = massiveMatches
          .where((m) => m.groupName == targetGroupName && m.status == 'finished')
          .toList();
    ...
    test('2. 【PDF性能】100ページの巨大プログラム冊子が読み込まれた際、メタデータの展開およびビューポート計算の内部プロセスタイムが、規定の「2sec (2000ms)」未満に収まること', () {
      ...
      final pages = List.generate(largePdfProgram.pageCount, (index) => 'page_render_context_$index');
    ...
    test('3. 【リーグ表性能】50チーム（総当たり最大1225総当たりセッション）規模の巨大集計マトリクスにおいて、勝率および順位計算のフォーマット処理が「200ms」未満で完了すること', () {
      final massiveStats = List.generate(50, (index) => PlayerStats('チーム・道場連盟_$index'));
