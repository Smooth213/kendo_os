...
class MockAppProjectionStore extends Mock
    implements app_store.ProjectionStore {}

class MockDomainProjectionStore extends Mock
    implements domain_store.ProjectionStore {}
...
void main() {
  ...
  setUp(() {
    ...
    container = ProviderContainer(
      overrides: [
        ...
        new_sync.syncEngineProvider.overrideWithValue(
          mockNewSyncEngine,
        ), // ★ モックを追加
        uiMessageProvider.overrideWith(
          () => MockUiMessageNotifier(),
        ), // ★ モックを追加
        app_store.projectionStoreProvider.overrideWithValue(
          mockAppProjectionStore,
        ), // ★ Isar Projection 警告を抑止
        in_memory_store.projectionStoreProvider.overrideWithValue(
          mockDomainStore,
        ), // ★ 追加: ドメイン層のProjectionStoreもモック化
        settingsProvider.overrideWith(() => MockSettingsNotifier()),
        matchListProvider.overrideWith(
          (ref) => ref.watch(mockMatchListProvider),
        ),
      ],
    );
  });
  ...
  group('MatchProvider - Step 3-2: 状態遷移の検証', () {
    test('processScoreEvent を呼び出した際、正しく保存処理と監査ログが実行されること', () async {
      ...
      verify(
        () => mockLocalRepo.saveMatch(
          any(that: isA<MatchModel>().having((m) => m.redScore, 'redScore', 1)),
        ),
      ).called(1);
    ...
    test('undoEvent を呼び出した際、保存処理が実行されること', () async {
      ...
      verify(
        () => mockLocalRepo.saveMatch(
          any(that: isA<MatchModel>().having((m) => m.redScore, 'redScore', 0)),
        ),
      ).called(1);
    });
  });
  ...
  group('MatchProvider - Step 3-3: rebuild最適化の検証', () {
    test('.select() により、監視対象外のプロパティ変更では通知が飛ばないこと', () {
      ...
      container.listen<bool>(settingsProvider.select((s) => s.strikeVib), (
        previous,
        next,
      ) {
        callCount++;
      ...
      notifier.state = notifier.state.copyWith(
        audioFeedbackMode: notifier.state.audioFeedbackMode == 'off'
            ? 'effect'
            : 'off',
      );
      expect(callCount, 0);
      ...
      notifier.state = notifier.state.copyWith(
        strikeVib: !notifier.state.strikeVib,
      );
      expect(callCount, 1);
    });
  });
  ...
  group('MatchProvider - Step 3-4: 同時操作・競合の防止', () {
    test('データ保存時に競合エラーが発生した場合、上位に例外が伝播すること', () async {
      ...
      ).thenThrow(Exception('他の端末でデータが更新されました'));
      ...
      verify(
        () => mockLocalRepo.saveMatch(
          any(
            that: isA<MatchModel>()
                .having(
                  (m) => m.calculateRemainingSeconds(now),
                  'remainingSeconds',
                  45,
                )
                .having((m) => m.status, 'status', 'finished'),
          ),
        ),
      ).called(1);
    });
