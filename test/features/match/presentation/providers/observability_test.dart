  ...
  group('🔍 Observability (可観測性) Tests', () {
    ...
    setUp(() {
      ...
      container = ProviderContainer(
        overrides: [alertProvider.overrideWithValue(mockAlertService)],
      );
    });
    ...
    test('1. メトリクス記録時に traceId が付与されること', () {
      ...
      expect(
        () => metrics.recordLatency('test_op', 150, traceId: traceId),
        returnsNormally,
      );
    ...
    test('3. エラー率が1%を超えた場合、HighErrorRateアラートが発行されること', () async {
      ...
      // この時点ではエラー0件
      verifyNever(
        () => mockAlertService.triggerAlert('HighErrorRate', any(), any()),
      );
