...
class FakeTournamentRepository implements TournamentRepository {
  final StreamController<TournamentModel?> _controller = StreamController<TournamentModel?>.broadcast();

  void emitData(TournamentModel? model) { _controller.add(model); }
...
void main() {
  group('🛡️ STEP 3-1: viewerTournamentProjectionProvider 非同期4状態 完全テスト要塞', () {
    ...
    test('1. 【Projection生成】正常な大会モデルと試合リストが供給された際、完全な動的Projection（AsyncData）が生成されること', () async {
    ...
    test('2. 【null処理】大会データが未登録（null）であっても、システムがクラッシュせずフォールバックダミーが自動適用されること', () async {
      final container = ProviderContainer(
        overrides: [
          tournamentRepositoryProvider.overrideWithValue(fakeRepo),
          matchListProvider.overrideWithValue([]),
    ...
    test('3. 【loading】インフラ層からのデータ疎通を待機している間、プロバイダが正確に AsyncLoading 状態をホールドすること', () {
      final container = ProviderContainer(
        overrides: [
          tournamentRepositoryProvider.overrideWithValue(fakeRepo),
          matchListProvider.overrideWithValue([]),
    ...
    test('4. 【error】通信障害や権限エラーが発生した際、プロバイダが破綻せず正確に AsyncError をUIへ伝播すること', () async {
      final container = ProviderContainer(
        overrides: [
          tournamentRepositoryProvider.overrideWithValue(fakeRepo),
          matchListProvider.overrideWithValue([]),
