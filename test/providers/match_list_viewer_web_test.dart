...
void main() {
  group('🛡️ Match List Viewer Web Initialization Test', () {
    ...
    test('✅ 1. matchListByTournamentProvider が initialization 中に他の provider を修正しないこと', () async {
      ...
      try {
        ...
        await testContainer.read(
          matchListByTournamentProvider('test_tournament_001').future
        ).catchError((e) {
      ...
      expect(initializationViolationDetected, isFalse,
        reason: 'matchListByTournamentProvider should not modify other providers during init');
    });

    test('✅ 2. Web 環境で tournament ID が ViewerHomeScreen.build() で初期化されること', () async {
      ...
      testContainer.read(webCurrentTournamentIdProvider.notifier).state = tournamentId;
      
      expect(testContainer.read(webCurrentTournamentIdProvider), equals(tournamentId),
        reason: 'Web tournament ID should be initialized after build completes');
    });

    test('✅ 3. webCurrentTournamentIdProvider と matchListByTournamentProvider が독립적으로 동작하는지 확인', () async {
      ...
      try {
        await testContainer.read(
          matchListByTournamentProvider(tournamentId).future
    ...
    test('✅ 4. Web キャッシュ (webCurrentTournamentMatchesProvider) が正しく保持されること', () async {
      ...
      testContainer.read(webCurrentTournamentMatchesProvider.notifier).state = testMatches;
    ...
    test('✅ 5. tournament ID 切り替え時に Web 画面が正しく更新されること', () async {
      ...
      await Future.delayed(Duration.zero);
      testContainer.read(webCurrentTournamentIdProvider.notifier).state = tournament1;
      expect(testContainer.read(webCurrentTournamentIdProvider), equals(tournament1));
      
      // Switch to tournament 2
      await Future.delayed(Duration.zero);
      testContainer.read(webCurrentTournamentIdProvider.notifier).state = tournament2;
      expect(testContainer.read(webCurrentTournamentIdProvider), equals(tournament2),
