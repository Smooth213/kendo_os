...
void main() {
  group('MatchUseCase - イベント駆動版テスト', () {
    ...
    setUp(() {
      ...
      rebuildUseCase = RebuildMatchFromEventsUseCase(engine, timeSource);
    ...
    test('ポイント追加によって MatchModel のスコアとイベント履歴が更新されること', () {
      ...
      // Act
      final updatedMatch = addScoreUseCase.execute(
        testUser,
        initialMatch,
        event,
        rule,
    ...
    test('Undoによって最新のイベントがキャンセル扱いになり、スコアが戻ること', () {
      ...
      // Act
      final undoneMatch = undoScoreUseCase.execute(
        testUser,
        matchWithScore,
        rule,
      );
      ...
      expect(
        undoneMatch.events.length,
        2,
        reason: '元イベント1件 + Undoイベント1件 で計2件になるべき',
      );
      expect(
        undoneMatch.events.last.isUndo,
        true,
        reason: '最新のイベントはUndoフラグが立っているべき',
      );
    ...
    test(
      ...
      () {
        ...
        // 2. 赤が1本目を取得
        final match1 = addScoreUseCase.execute(
          testUser,
          initialMatch,
          men(Side.red),
          rule,
        ...
        final match2 = addScoreUseCase.execute(
          testUser,
          match1,
          kote(Side.red),
          rule,
        ...
        expect(
          match2.status,
          'finished',
          reason: '2本取得した時点で試合は終了(finished)にならなければならない',
        );
        ...
        expect(
          undoneMatch.status,
          'in_progress',
          reason: '終了状態が解除され、再び進行中(in_progress)に戻っていること',
        );
      },
    );
