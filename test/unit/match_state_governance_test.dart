...
void main() {
  group('🛡️ Phase 3 — 状態遷移保証ユニットテスト要塞', () {
    ...
    test('1. 【FSM・状態遷移関数】transitionメソッドを介したライフサイクル状態の遷移が決定論的にstatus文字列へ投影されること', () {
      ...
      final playingMatch = baseMatch.transition(MatchLifecycleState.inProgress);
      expect(playingMatch.status, equals('in_progress'));

      final finishedMatch = playingMatch.transition(MatchLifecycleState.completed);
      expect(finishedMatch.status, equals('finished'));
    });

    test('2. 【タイマー状態遷移・絶対時間】経過時間Msと外部注入タイムソース(now)から残り秒数が1ミリ秒の狂いもなく絶対プロジェクションされること', () {
    ...
    test('3. 【Undo操作保護】Strangler Figパターンで保護されたisDirtyフラグおよび同等の状態ライフサイクルが非破壊原則を維持していること', () {
      final syncedMatch = MatchBuilder().id('undo_fsm_001').build();
      expect(syncedMatch.isDirty, isFalse);

      final localDirtyMatch = syncedMatch.copyWith(syncState: SyncState.localOnly);
