...
void main() {
  group('🛡️ Phase 1-5: MatchStateMachine Impossible State FSM Tests', () {
    test('✅ 正常な遷移: notStarted -> ready -> inProgress', () {
      ...
      final state2 = MatchStateMachine.transition(
        state1,
      ...
      expect(
        ...
        reason: '試合開始によりinProgressへ遷移すること',
      );
    });
    ...
    test(
      ...
      () {
        ...
        bool isInfinityBlocked =
            rawRemainingInfinity.isInfinite || rawRemainingInfinity.isNaN;
        ...
        expect(
          () => match.calculateRemainingSeconds(DateTime.now()),
          returnsNormally,
          reason:
              'メソッド内部で Unsupported operation: Infinity or NaN toInt 例外が発生してはなりません',
        );
      },
    );
  });
