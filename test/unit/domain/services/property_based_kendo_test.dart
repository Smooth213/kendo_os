  ...
  group('🧪 Phase 3-1: Property-based testing (不変条件の自動検証)', () {
    test('10,000回のランダムな試合展開（技・反則・Undo）でも不変条件が絶対に壊れないこと', () {
      for (int i = 0; i < 10000; i++) {
        ...
        final rule = MatchRule(ipponLimit: ipponLimit, matchTimeMinutes: 3.0);
        ...
        for (int j = 0; j < eventCount; j++) {
          ...
          final isFinished =
              currentAnalysis.context.redIppon >= rule.ipponLimit ||
              currentAnalysis.context.whiteIppon >= rule.ipponLimit;
          ...
          if (isUndo) {
            ...
            events[targetIndex] = events[targetIndex].copyWith(
              isCanceled: true,
            );
          } else {
            // 通常のイベント追加
        ...
        final expectedWhiteIpponFromHansoku =
            context.redHansoku ~/ rule.hansokuLimit;
        final expectedRedIpponFromHansoku =
            context.whiteHansoku ~/ rule.hansokuLimit;

        expect(
          context.whiteIppon,
          greaterThanOrEqualTo(expectedWhiteIpponFromHansoku),
          reason:
              '赤の反則(${context.redHansoku}回)による白への得点加算が不足しています (Iteration: $i)',
        );
        expect(
          context.redIppon,
          greaterThanOrEqualTo(expectedRedIpponFromHansoku),
          reason:
              '白の反則(${context.whiteHansoku}回)による赤への得点加算が不足しています (Iteration: $i)',
        );
