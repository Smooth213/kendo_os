  ...
  group('🛡️ Web Viewer Screen Initialization Test', () {
    test('✅ 1. kIsWeb フラグが Web 環境で正しく動作すること', () {
      ...
      expect(
        kIsWeb,
        isA<bool>(),
        reason: 'kIsWeb should be a boolean constant',
      );
    });
    ...
    test('✅ 2. ViewerHomeScreen が Web 環境用 tournament ID 初期化ロジックを持つこと', () async {
      ...
      expect(
        executionOrder,
        containsAll(['build_start', 'build_end']),
      ...
      if (kIsWeb) {
        expect(
          ...
          isTrue,
          reason:
              'Web tournament ID initialization should use deferred callback',
        );
      }
    });
    ...
    test('✅ 3. tournament ID が変更時のみ再初期化されること（パフォーマンス最適化）', () {
      ...
      expect(
        updateCount,
        equals(1),
        reason: 'First update should increment count',
      );
      ...
      expect(
        updateCount,
        equals(1),
        reason: 'Duplicate update should not increment',
      );
      ...
      expect(
        updateCount,
        equals(2),
        reason: 'Different tournament should increment',
      );
    });
    ...
    test('✅ 5. Web/Native 判断ロジックが正確に実装されたことを確認', () {
      ...
      expect(
        selectedProvider.toLowerCase().contains('tournament'),
        isTrue,
      ...
      if (isWeb) {
        expect(
          selectedProvider.contains('web'),
          isTrue,
          reason: 'Web provider should contain "web"',
        );
      } else {
        expect(
          selectedProvider.toLowerCase().contains('current'),
          isTrue,
    ...
    test('✅ 6. Firestore 쿼리가 tournament ID로 필터링되는지 확인', () async {
      ...
      expect(
        query,
        contains('WHERE'),
        reason: 'Query should have WHERE clause',
      );
    });
