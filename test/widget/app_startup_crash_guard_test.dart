...
void main() {
  group('🛡️ アプリ起動エラー（シミュレータ起動不可・ホワイトアウト）保護テスト', () {
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final Exception mockFatalError = Exception(
          'IsarError: Cannot initialize Isar core in simulator.',
        );
        ...
        expect(
          find.textContaining('【ブラウザのセキュリティ制限】', skipOffstage: false),
          findsOneWidget,
        );
        expect(
          find.textContaining('LINEやQRコードリーダーの内蔵ブラウザ', skipOffstage: false),
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets('2. その他のエラー([core/no-app]等)発生時は、通常のエラー原因と詳細スタックが描画されること', (
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Scaffold(
            body: Center(
              child: Text(displayMessage, textAlign: TextAlign.center),
            ),
          ),
        ),
      );
      ...
      expect(
        find.textContaining('アプリの起動に失敗しました。', skipOffstage: false),
        findsOneWidget,
      );
      expect(
        find.textContaining('[core/no-app]', skipOffstage: false),
        findsOneWidget,
      );
    });
    ...
    testWidgets('3. FlutterError.onError のカスタムエラー画面（X線画面）が正しく描画されること', (
      ...
      expect(
        find.textContaining('Simulated RenderFlex Overflow Error'),
        findsOneWidget,
      );
    });
  });
