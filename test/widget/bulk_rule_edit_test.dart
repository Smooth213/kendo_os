...
void main() {
  group('BulkRuleEditSheet Widget Tests', () {
    ...
    testWidgets('1. Render options, checkboxes, filters, and update successfully', (
      WidgetTester tester,
    ) async {
      // Enlarge physical size to see all elements
      ...
      // Keep track of bulk update executions
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            // Override MatchCommandService/matchCommandProvider to intercept bulkUpdateMatchRules
      ...
      // Check if all 3 match descriptions are present (without suffixes because of grouping and mapping)
      ...
      // Tap the option '小学生低学年の部'
      await tester.tap(find.text('小学生低学年の部').last);
      await tester.pumpAndSettle();

      // Verify count decreases to 2 (only match_1 and match_3 match '小学生低学年の部')
      ...
      // Find CheckboxListTile for match_3 and uncheck it
      final checkboxFinder = find.byType(CheckboxListTile);
      expect(checkboxFinder, findsNWidgets(2));

      // Tap the checkbox for '鈴木 三郎' to uncheck it
      ...
      // 7. Adjust rules inside the sheet:
      // Set Match Time to 2.0 minutes
      await tester.tap(find.text('3分').first);
      await tester.pumpAndSettle();
      await tester.tap(find.text('2分').last);
      await tester.pumpAndSettle();

      // Toggle Hantei ON
      await tester.tap(find.text('判定（ハンテイ）の適用'));
      await tester.pumpAndSettle();
      ...
      expect(lastUpdatedIds, ['match_1']); // only match_1 was selected
