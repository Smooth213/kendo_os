...
void main() {
  group('🛡️ フェーズ5: 統合テスト (Integration Test) 究極防衛要塞', () {
    ...
    test('1. 【カオスネットワーク・Eventual Consistency】分散ノードから乱序で届いたイベントが論理時計で一本の鎖にマージされ、状態ドリフトが完全ゼロに収束すること', () {
    ...
    test('2. 【決定論的リプレイ】同一のイベント履歴ストアから生成されたMatchModelは、どの分散端末上で何度リプレイしても1ビットの狂いもなく完全一致すること', () {
      final eventsHistory = [
        ScoreEvent(
          id: 'h1',
          side: Side.red,
      ...
      expect(matchStateTerminalX.events.length, equals(matchStateTerminalY.events.length));
      expect(matchStateTerminalX.events.first.id, equals(matchStateTerminalY.events.first.id));
      expect(matchStateTerminalX.events.last.id, equals(matchStateTerminalY.events.last.id));
    });
  });
}