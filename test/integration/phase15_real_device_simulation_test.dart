...
void main() {
  group('🛡️ PHASE 15 — リアル端末マルチOS・物理境界エミュレーションテスト要塞', () {
    ...
    final devices = {
      'iPhone_SE_Variant': {'size': const Size(375, 667), 'padding': const EdgeInsets.only(top: 20, bottom: 0), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'iPhone_15_Variant': {'size': const Size(393, 852), 'padding': const EdgeInsets.only(top: 59, bottom: 34), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'iPhone_17_Pro_Variant': {'size': const Size(402, 874), 'padding': const EdgeInsets.only(top: 62, bottom: 34), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'Android_Pixel_Variant': {'size': const Size(412, 915), 'padding': const EdgeInsets.only(top: 24, bottom: 0), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'Android_Galaxy_Variant': {'size': const Size(360, 800), 'padding': const EdgeInsets.only(top: 24, bottom: 0), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'iPhone_15_Keyboard_Active': {'size': const Size(393, 852), 'padding': const EdgeInsets.only(top: 59, bottom: 34), 'insets': const EdgeInsets.only(bottom: 336), 'isKeyboard': true},
      'iPad_Mini_Variant': {'size': const Size(768, 1024), 'padding': const EdgeInsets.only(top: 20, bottom: 0), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'iPad_Pro_11_Variant': {'size': const Size(834, 1194), 'padding': const EdgeInsets.only(top: 24, bottom: 20), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'iPad_Pro_12.9_Variant': {'size': const Size(1024, 1366), 'padding': const EdgeInsets.only(top: 24, bottom: 20), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'Samsung_Tab_S8_Variant': {'size': const Size(800, 1280), 'padding': const EdgeInsets.only(top: 24, bottom: 0), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'Pixel_Tablet_Variant': {'size': const Size(800, 1280), 'padding': const EdgeInsets.only(top: 24, bottom: 0), 'insets': EdgeInsets.zero, 'isKeyboard': false},
      'Desktop_FHD_Variant': {'size': const Size(1920, 1080), 'padding': EdgeInsets.zero, 'insets': EdgeInsets.zero, 'isKeyboard': false},
    };
          ...
          createUnifiedTestableWidget(
            MediaQuery(
              ...
              child: Scaffold(
                ...
                body: SingleChildScrollView(
                  child: Column(
                    children: [
                      SizedBox(height: 400, child: const MatchScreen(matchId: testMatchId)),
                      SizedBox(
                        height: 400, 
            ...
            overrides: [
              ...
              matchStreamProvider.overrideWith((ref) => Stream<List<MatchModel>>.value([
                MatchModel(id: testMatchId, tournamentId: 'dummy_t1', matchType: '個人戦', redName: '赤選手', whiteName: '白選手', groupName: '一般の部', status: 'waiting')
              ])),
              matchListProvider.overrideWith((ref) => [
                MatchModel(id: testMatchId, tournamentId: 'dummy_t1', matchType: '個人戦', redName: '赤選手', whiteName: '白選手', groupName: '一般の部', status: 'waiting')
              ]),
              strokeRepositoryProvider.overrideWith((ref) => FakeStrokeRepository()),
              localStrokeRepositoryProvider.overrideWith((ref) => FakeLocalStrokeRepository()),
              matchViewStateUserIdProvider.overrideWith((ref) => 'test_user_id'),
              matchApplicationServiceProvider.overrideWithValue(FakeMatchApplicationService()),
            ],
          ),
        );
        await tester.pump(const Duration(milliseconds: 500)); // 🛡️ 無限アニメーション待ちを回避し、安全に終了させる
