...
void main() {
  group('🛡️ PHASE 14 — 大会当日運営シナリオE2E・現場カオス完全制覇テスト要塞', () {
    ...
    test(
      '1. 【シナリオ1: 団体リーグ戦】 大会作成 ➔ リーグ作成 ➔ 複数試合進行 ➔ 順位確定 ➔ 結果出力までの全ステートが独立性を保ち正常に遷移完了すること',
      () {
        final match1 = MatchBuilder()
            .id('s1_match_1')
            .groupName('予選リーグ_Aブロック')
            .matchType('先鋒')
            .build();
        final match2 = MatchBuilder()
            .id('s1_match_2')
            .groupName('予選リーグ_Aブロック')
            .matchType('次鋒')
            .build();
    ...
    test(
      '2. 【シナリオ2: 勝ち抜き代表戦】 勝ち抜き戦の全ポジション終了後に同点となり、代表戦(無制限延長)へ突入 ➔ 決着による大会終了ステートへ数学的に収束すること',
      () {
    ...
    test(
      '3. 【シナリオ3: 個人戦】 個人戦トーナメント ➔ 予選リーグ ➔ 3位決定戦・決勝戦にいたる独立リレーショナル状態がデグレを起こさずプロジェクションされること',
      () {
        final finalMatch = MatchBuilder()
            .id('s3_individual_final')
            .matchType('個人戦_決勝')
            .build();
        final playOffMatch = MatchBuilder()
            .id('s3_individual_3rd')
            .matchType('個人戦_3位決定戦')
            .build();
    ...
    test(
      '4. 【シナリオ4: 100試合同期保留バースト合流】 完全オフライン状態で100試合分のスコア入力を未送信（localOnly）のままキューに保留し、オンライン復旧時に1ビットの欠落もなく一括バースト同期が完了すること',
      () {
        final pendingQueue = List.generate(
          100,
          (index) => MatchBuilder()
        ...
        final syncedQueue = pendingQueue
            .map((m) => m.copyWith(syncState: SyncState.synced))
            .toList();
