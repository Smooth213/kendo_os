...
void main() {
  group('🛡️ Phase 6 — PWA/Web耐久・ブラウザカオス耐性テスト要塞', () {
    ...
    test(
      '1. 【IndexedDB容量超過・破損】ブラウザストレージが容量上限(QuotaExceeded)またはキャッシュ破損を起こしても、メモリ内の不変ドメイン状態がクラッシュせず安全に維持されること',
      () {
    ...
    test(
      '2. 【ServiceWorker競合】新旧のServiceWorkerがブラウザキャッシュ上で一時的に混在し、同一イベントが異なる経由で2回検知されても、論理クロックの同一性により重複履歴が完全にマージ排除されること',
      () {
    ...
    test(
      '3. 【Multi-tab同時入力】記録員が誤って2つのブラウザタブを開き、双方から同時に異なるスコアイベントが入力されても、論理時計ソート契約により歴史が確定的一本化すること',
      () {
