...
void main() {
  group('🛡️ Isar Integer JavaScript Safety Test', () {
    ...
    Future<List<(String, int, String)>> findUnsafeIntegers(String filePath) async {
    ...
    test('✅ 1. match_entity.g.dart が JS-safe 整数のみを使用すること', () async {
      const filePath = 'lib/infrastructure/persistence/models/match_entity.g.dart';
      final unsafeIds = await findUnsafeIntegers(filePath);
      
      expect(unsafeIds, isEmpty,
        reason: 'match_entity.g.dart contains integers exceeding JavaScript safe range:\n'
                '${unsafeIds.map((e) => 'Line ${e.$2}: ${e.$3}').join('\n')}');
    });

    test('✅ 2. local_stroke_model.g.dart が JS-safe 整数のみを使用すること', () async {
      const filePath = 'lib/infrastructure/persistence/models/local_stroke_model.g.dart';
      final unsafeIds = await findUnsafeIntegers(filePath);
      
      expect(unsafeIds, isEmpty,
        reason: 'local_stroke_model.g.dart contains integers exceeding JavaScript safe range:\n'
                '${unsafeIds.map((e) => 'Line ${e.$2}: ${e.$3}').join('\n')}');
    });

    test('✅ 3. match_comment_entity.g.dart が JS-safe 整数のみを使用すること', () async {
      const filePath = 'lib/infrastructure/persistence/models/match_comment_entity.g.dart';
      final unsafeIds = await findUnsafeIntegers(filePath);
      
      expect(unsafeIds, isEmpty,
        reason: 'match_comment_entity.g.dart contains integers exceeding JavaScript safe range:\n'
                '${unsafeIds.map((e) => 'Line ${e.$2}: ${e.$3}').join('\n')}');
    });

    test('✅ 4. match_projection_entity.g.dart が JS-safe 整数のみを使用すること', () async {
      const filePath = 'lib/infrastructure/persistence/models/match_projection_entity.g.dart';
      final unsafeIds = await findUnsafeIntegers(filePath);
      
      expect(unsafeIds, isEmpty,
        reason: 'match_projection_entity.g.dart contains integers exceeding JavaScript safe range:\n'
                '${unsafeIds.map((e) => 'Line ${e.$2}: ${e.$3}').join('\n')}');
    });
    ...
    test('✅ 5. 全 .g.dart ファイルが制限内の整数を使用すること（包括テスト）', () async {
      ...
      expect(allUnsafe, isEmpty,
        reason: 'Generated model files contain unsafe integers:\n'
    ...
    test('✅ 7. 生成後再実行しても安全性が維持されることを確認（再発防止）', () async {
      ...
      for (final filePath in files) {
        final unsafeIds = await findUnsafeIntegers(filePath);
        expect(unsafeIds, isEmpty,
          reason: 'REGRESSION: $filePath has lost JS-safe ID patches after generation. '
    ...
    test('✅ 8. build_runner 再実行後 .backup_g_dart の復元が完了することを確認', () {
      ...
      if (deployScript.existsSync()) {
        ...
        expect(content.contains('backup_g_dart'), isTrue,
          reason: 'deploy_web.sh should contain backup/restore logic for .g.dart files');
      }
    });
  });
}
